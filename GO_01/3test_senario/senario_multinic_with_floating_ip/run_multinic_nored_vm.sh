...
source ./5make_multinic_vm_with_floating_ip_for_mgmt.sh
    ...
    make_user_multinic_nored_utm_vm ${customer}_nored_utm02 $UTM_IMAGE \
        global_mgmt_net $GREEN_NET $ORANGE_NET \
        $AVAILABILITY_ZONE $CNODE01
    
    allocate_floating_ip_to_mgmt_vm ${customer}_nored_utm02
