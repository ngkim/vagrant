...
purge_vm()
{
    echo '---------------------------------------------------------------------'
    echo '## purge_vm'
    for vm in `nova list | egrep -v '\-\-|ID' | awk '{print $2}'`
    do
        echo '  -> ' ${vm}
        echo "nova delete ${vm}"
        nova delete ${vm}
...
{
    echo '---------------------------------------------------------------------'
    echo "## customer tenant($GUEST_TENANT_NAME)/user($GUEST_USER_NAME) 삭제"
    
    if [[ -z "$GUEST_TENANT_NAME" || -z "$GUEST_USER_NAME" ]]; then
        echo "Error: GUEST_TENANT_NAME/GUEST_USER_NAME 변수중 하나가 미 선언됨!!!"
        exit        
    fi
    
    # 계정삭제할 때는 admin환경으로 설정
    set_openstack_cli_admin_env
    
    echo "keystone tenant-delete $GUEST_TENANT_NAME"
    keystone tenant-delete $GUEST_TENANT_NAME
    echo "keystone user-delete $GUEST_USER_NAME"
    keystone user-delete $GUEST_USER_NAME
    
    # 사고예방을 위해 user 환경으로 설정
    set_openstack_cli_user_env
    echo '---------------------------------------------------------------------'
}
